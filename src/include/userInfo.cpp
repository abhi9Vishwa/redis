...
void UserInfo::allowUserOnAuth(int& fd) {
    this->allowedUsers.insert(fd);
}

bool UserInfo::isUserAllowed(int& fd) {
	return this->allowedUsers.find(fd) != this->allowedUsers.end();
}
...
bool UserInfo::isNoPassSet() {
    return this->flags.find("nopass") != this->flags.end();
}